...
export const useLoyaltyCards = () => {
  ...
  watchDebounced(cards, (c) => localStorage.setItem(useLoyaltyCardskey, PublicSign(c)), { debounce: 200 })
...
export const setTheatre = (id: number) => {
  useCookie<number>('uec-theatre', { expires: dayjs().add(1, 'year').toDate() }).value = id
}
