...
const linkRules: Record<string, RuleFn[]> = {
    email: [
        (value: string) => {
...
<template>
    <p class="dark:text-white/50">
        All your Loyalty Rewards cards in one place. You can link, update, or
        remove a linked Loyalty Rewards card. FREE Loyalty Rewards cards can
        only be obtained in-person at the UEC Theatre Box Office with your name,
    ...
    <div class="grid sm:grid-cols-2 md:grid-cols-3 gap-4">
        ...
        <button
            :class="[
                'aspect-video flex justify-center items-center gap-2 transition duration-200',
    ...
    <u-modal
        v-model:visible="link.enabled"
        title="Link Loyalty Rewards Card"
        :bg-closable="false"
    >
        <u-form :model="link" :rules="linkRules">
            <u-form-item label="Card Number" required>
                <u-input v-model:value="link.number" />
            </u-form-item>

            <u-form-item label="Full Name" required>
                <u-input v-model:value="link.name" />
            </u-form-item>

            <u-form-item label="Email" name="email" required>
                <u-input v-model:value="link.email" />
            </u-form-item>

            <u-form-item label="Street Address" required>
                <u-input v-model:value="link.street" />
            </u-form-item>

            <div class="grid grid-cols-2 gap-4">
                <u-form-item label="City" required>
                    <u-input v-model:value="link.city" />
                </u-form-item>

                <u-form-item label="State" required>
                    <u-select v-model:value="link.state" :options="states" />
                </u-form-item>
            </div>

            <div class="grid grid-cols-2 gap-4">
                <u-form-item label="Zip Code" required>
                    <u-input v-model:value="link.zip" />
                </u-form-item>

                <u-form-item label="Phone" required>
                    <u-input v-model:value="link.phone" />
                </u-form-item>
            </div>

            <u-form-item>
                <u-button>Link</u-button>
            </u-form-item>
        </u-form>
    </u-modal>
</template>
