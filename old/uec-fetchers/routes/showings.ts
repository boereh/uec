...
async function DoDates(args: DoDatesArgs) {
  const datedom = new JSDOM(
    await $fetch<string>(`${args.url}?showdate=${args.date}`)
  );
  const LiElements =
    datedom.window.document.querySelectorAll<HTMLLIElement>(
      "#now-playing > li"
    );
  let inserted = 0;

  for (const el of LiElements) {
    const times = el.querySelectorAll<HTMLSpanElement>(
      ".auditoriumStyleShowtimes > span"
    );
    const movie_id = Number(
      el
        .querySelector<HTMLAnchorElement>(".movieTimes > .movieTimesLeft > a")
        .href.split("/")
        .at(-1)
    );
    ...
    for (const time of times) {
      ...
      const date = dayjs
        .tz(args.date, args.theatre.timezone)
        .add(seconds, "second");
      ...
      const index = args.showings.findIndex((x) => {
        const date =
          dayjs.tz(showing.date, args.theatre.timezone).toISOString() ===
          dayjs.tz(x.date, args.theatre.timezone).toISOString();
...
export default defineEventHandler(async () => {
  ...
  for (const item of fetched) {
    ...
    const dates = dom.window.document.querySelectorAll<HTMLOptionElement>(
      "select#showdate > option"
    );
